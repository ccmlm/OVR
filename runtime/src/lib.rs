...
pub use sp_runtime::{DispatchResult, Perbill, Percent, Permill, Perquintill};
use sp_std::prelude::*;
...
pub use frame_support::{
    ...
    traits::{
        schedule::Priority, EnsureOrigin, KeyOwnerProofSystem, OriginTrait, Randomness, U128CurrencyToVote,
        WithdrawReasons,
    },
    weights::{
...
pub use primitives::{
    evm::EstimateResourcesRequest, AccountId, AccountIndex, Amount, AuthoritysOriginId, Balance, BlockNumber,
    CurrencyId, EraIndex, Hash, Moment, Nonce, Signature, TokenSymbol,
};

pub use runtime_common::{
    BlockLength, BlockWeights, GasToWeight, OffchainSolutionWeightLimit, Price, Rate, Ratio, SystemContractsFilter,
};
...
parameter_types! {
    ...
    // pub TreasuryModuleAccount: AccountId = OvrTreasuryModuleId::get().into_account();
}
...
impl orml_authority::AuthorityConfig<Origin, OriginCaller, BlockNumber> for AuthorityConfigImpl {
    fn check_schedule_dispatch(origin: Origin, _priority: Priority) -> DispatchResult {
        EnsureRoot::<AccountId>::try_origin(origin).map_or_else(|_| Err(BadOrigin.into()), |_| Ok(()))
    }
        ...
        ensure_root(origin.clone()).or_else(|_| {
            match self {
                AuthoritysOriginId::Root => {
                    <EnsureDelayed<SevenDays, EnsureRoot<AccountId>, BlockNumber, OriginCaller> as EnsureOrigin<
                        Origin,
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
    spec_name: create_runtime_str!("ovr"),
    impl_name: create_runtime_str!("ovr"),
    authoring_version: 1,
    spec_version: 9,
    impl_version: 9,
...
impl frame_system::Config for Runtime {
    ...
    /// This is a hook that is use when setCode is called - not require unless
    /// using cumulus.
    type OnSetCode = ();
}
...
parameter_types! {
    pub const BasicDeposit: Balance =      100 * OVR;
    pub const FieldDeposit: Balance =        1 * OVR;
    pub const SubAccountDeposit: Balance =  20 * OVR;
...
parameter_types! {
    pub const IndexDeposit: Balance = 1 * OVR;
}
...
parameter_types! {
    pub const TransactionByteFee: Balance = 10 * MILLI_OVR;
...
parameter_types! {
    ...
    // 10 OVR minimum storage deposit
    pub const NewContractExtraBytes: u32 = 10_000;
    pub const StorageDepositPerByte: Balance = 1 * MILLI_OVR;
    pub const MaxCodeSize: u32 = 60 * 1024;
    pub NetworkContractSource: H160 = H160::from_low_u64_be(0);
    pub const DeveloperDeposit: Balance = 1_000 * OVR;
    pub const DeploymentFee: Balance    = 100 * OVR;
}
...
parameter_types! {
    // note: if we add other native tokens (OUSD) we have to set native
    // existential deposit to 0 or check for other tokens on account pruning
    pub const NativeTokenExistentialDeposit: Balance =       1 * OVR;
    pub const MaxNativeTokenExistentialDeposit: Balance = 1000 * OVR;
...
parameter_types! {
    ...
    pub const CandidacyDeposit: Balance =   1_000_000 * primitives::currency::OVR;
    pub const MinLockAmount: Balance =        100_000 * primitives::currency::OVR;
    pub const TotalLockedCap: Balance = 2_000_000_000 * primitives::currency::OVR;
}
...
// TODO: Implementation of `From` is preferred since it gives you `Into<_>` for
// free where the reverse isn't true. After this TODO will be resolved, remove
// the suppresion of `from-over-into` warnings in the Makefile.
construct_runtime!(
